...
export default class {
  ...
  */
  ...
  calculatePeaks(samplesPerPixel, sampleRate) {
    ...
    const peaks = extractPeaks(this.buffer, samplesPerPixel, this.peakData.mono, cueIn, cueOut);
    this.setPeaks(peaks);
  }

  fakeCutPeaks(samplesPerPixel, sampleRate, start, end) {
    const cueIn = secondsToSamples(this.cueIn, sampleRate);
    const cueOut = secondsToSamples(this.cueOut, sampleRate);
    const peaks = extractPeaks(this.buffer, samplesPerPixel, this.peakData.mono, cueIn, cueOut);

    //One peak units in buffer units
    const pUnit = peaks.length / this.buffer.length;
    const pStart = Math.floor(start * pUnit);
    const pEnd = Math.ceil(end * pUnit);
    const pLength = pEnd - pStart;
    peaks.data[0] = peaks.data[0].filter((value, index) => {
      return index < pStart * 2 || index > (pEnd * 2)-3;
    });
    peaks.length = peaks.data[0].length / 2;
    this.setPeaks(peaks);
  }
