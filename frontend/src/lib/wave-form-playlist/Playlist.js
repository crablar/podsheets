/// <reference path="worker-loader.d.ts"/>
...
let mp3Worker = require("worker-loader!./exportWavWorker.worker.js");
...
export default class {
  ...
  initExporter() {
    //const ExportWavWorker = require('worker-loader!./exportWavWorker.js');
    this.exportWorker = new mp3Worker();//new InlineWorker(ExportWavWorkerFunction);
  }
  ...
  setUpEventEmitter() {
    ...
    ee.on('trim', () => {
      ...
      tracks.forEach((track) => {
        ...
        if (timeSelection.start < 0) {
          timeSelection.start = 0;
        }

        const duration = track.buffer.duration;
        const dUnit = track.buffer.length / track.buffer.duration;

        const start = dUnit * timeSelection.start;
        const end = dUnit * timeSelection.end;
        track.fakeCutPeaks(this.samplesPerPixel, this.sampleRate, start, end);
        setTimeout(() => {
          const result = utils.cut(track.buffer, start, end);
          track.setBuffer(result);
          track.playout.buffer = result;
          track.endTime = result.duration;
          this.cursor = 0;
          this.stop();
    ...
    */
    this.offlineAudioContext.startRendering().then((audioBuffer) => {
      ...
      /*
            if (type === 'wav') {
              ...
              this.exportWorker.onmessage = (e) => {
                // console.warn("WebWorker message data", e.data);
                this.ee.emit('audiorenderingfinished', "mp3", e.data);
                console.warn(e.data);
                this.isRendering = false;
              };
      ...
      */
      ...
      let mp3Tmp = mp3encoder.encodeBuffer(audioBuffer); //encode mp3
      ...
      this.ee.emit('audiorenderingfinished', "mp3", blob);
      /*
      // init gets a global handle to refer to a single encoding session.
      lame.init((error, handle) => {
        ...
        lame.encodeBuffer(handle, array, array, (error, buffer) => {
          ...
          lame.encodeFlush(handle, (error, buffer) => {
            buffers.push(buffer);
            ...
            this.ee.emit('audiorenderingfinished', "audio/mp3", blob);
  ...
  */
  ...
  renderTrackSection() {
    const trackElements = this.tracks.map(track =>
      track.render(this.getTrackRenderData({
        isActive: this.isActiveTrack(track) || this.isActiveAll,
        shouldPlay: this.shouldTrackPlay(track),
  ...
  render() {
    ...
    return h('div.playlist',
      {
        attributes: {
          style: 'margin-left: 0px; overflow-x: scroll; position: relative;',
        },
      },
      containerChildren,
    );
